...
fn build_menu() -> SystemTrayMenu {
    let menuitem_quit = CustomMenuItem::new("quit".to_string(), "Quit").accelerator("Cmd+Q");

    let menuitem_settings =
        CustomMenuItem::new("settings".to_string(), "Settings").accelerator("Cmd+,");

    let submenu = SystemTraySubmenu::new(
        "More",
        SystemTrayMenu::new()
            .add_item(CustomMenuItem::new("sub-item-1".to_string(), "Sub item 1"))
            .add_item(CustomMenuItem::new("sub-item-2".to_string(), "Sub item 2")),
    );
